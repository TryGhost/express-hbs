    ...
    if (val.indexOf('__aSyNcId_') >= 0) {
      if (self.asyncValues) {
        Object.keys(self.asyncValues).forEach(function (id) {
          val = val.replace(id, function() {
            return self.asyncValues[id];
          });
          val = val.replace(self.Utils.escapeExpression(id), function() {
            return self.Utils.escapeExpression(self.asyncValues[id]);
          });
        });
      }
    }
