...
ExpressHbs.prototype.registerHelper = function(name, fn) {
  ...
  this.handlebars.registerHelper.call(this.handlebars, name, wrapper);
};

ExpressHbs.prototype.registerPartial = function(name, source) {
  if (typeof source !== 'string') {
    // A comment only partial causes errors. This must be a string
    // so we can append a space to end of it.
    throw new Error('registerPartial must be of type string due to bug in handlebars');
  }
  if (source.indexOf('}}') === (source.length - 2)) {
    source += ' ';
  }
  this.handlebars.registerPartial(name, this.handlebars.compile(source));
};

ExpressHbs.prototype.registerAsyncHelper = function(name, fn) {
  this.handlebars.registerHelper(name, function(context) {
    var result = async.resolve(fn.bind(this), context);
