...
ExpressHbs.prototype.___express = function ___express(filename, source, options, cb) {
  ...
  function renderTemplate(template, locals, cb) {
    ...
    try {
      var localTemplateOptions = locals._templateOptions || {};
      delete locals._templateOptions;
      res = template(locals, _.merge({}, self._options.templateOptions, localTemplateOptions));
    } catch (err) {
      if (err.message) {
